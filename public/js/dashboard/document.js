...
$(function () {
    ...
    var tableEx = $('#tableExport').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/dashboard/fetch-files',
        datatype: 'json',
        datasrc:'data',
        "columns": [
            {"mData": "title", "defaultContent": ''},
            {"mData": "body", "defaultContent": ''},
            {"mData": "filePath", "defaultContent": ''},
            {
    ...
    // search
    tableEx = $('#tableExport').DataTable();
    $("#tourSearch")
        .off()
        .on('keyup change', function (e) {
            if (e.keyCode == 13 || this.value == "") {
                tableEx.search(this.value)
                    .draw();
            }
        })
    ...
    /* $('#tourSearch').on('keyup',function (e) {
         ...
         $.ajax({
             url: '/dashboard/tour-search',
             data: {
                 _token: _token,
                 searchVal: $('#tourSearch').val(),
             },
             success: function (data) {
                 tableEx.search( data.data[0] ).draw();
             },
         });
     })*/
    /* $('#tourSearch').on('keyup',function (e){
         let _token   = $('meta[name="csrf-token"]').attr('content');
         var searchVal = $('#tourSearch').val();
...
function addColNumbers() {
    $('#tableExport thead tr').prepend('<th>شماره</th>');
